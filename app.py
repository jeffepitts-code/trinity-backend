...
async def spxw_flow(exp: str = Query(..., description="Expiration date YYYY-MM-DD")):
    ...
    url = POLYGON_BASE + "/v3/snapshot/options/I:SPX?limit=250&apiKey=" + MASSIVE_API_KEY
    async with httpx.AsyncClient(timeout=20) as client:
        try:
            resp = await client.get(url)
            resp.raise_for_status()
        except httpx.HTTPStatusError as e:
            raise HTTPException(status_code=502, detail="Massive API error: " + str(e.response.status_code))
        except httpx.RequestError as e:
            raise HTTPException(status_code=502, detail="Network error: " + str(e))
    data = resp.json()
    if data.get("status") == "ERROR":
        raise HTTPException(status_code=502, detail="Massive: " + data.get("error", "unknown error"))
    ...
    if not spot:
        async with httpx.AsyncClient(timeout=10) as client:
            try:
                pr = await client.get(POLYGON_BASE + "/v2/aggs/ticker/I:SPX/prev?adjusted=true&apiKey=" + MASSIVE_API_KEY)
    ...
    if not spxw:
        return JSONResponse({
            "ok": True,
            "empty": True,
            "reason": "No SPXW contracts for " + exp,
